...
class gameSetupTests {
	...
	void testPlayerSetup() {
		...
		assertEquals( player.getStartRow(), 8);
		assertEquals( player.getStartCol(), 1);
		
		// test that Darth Vader is in the correct location
		player = board.getPlayer("Darth Vader");
		assertTrue( player != null );
		assertEquals(player.getStartRow(), 13);
		assertEquals(player.getStartCol(), 26);
		
		// test that Han Solo is in the correct location
		player = board.getPlayer("Han Solo");
		assertTrue( player != null );
		assertEquals(player.getStartRow(), 1);
		assertEquals(player.getStartCol(), 5);
	}
